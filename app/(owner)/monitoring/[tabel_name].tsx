// app/(owner)/monitoring/[table_name].tsx
...
export default function TableMonitoringScreen() {
  const router = useRouter();
  const { table_name } = useLocalSearchParams<{ table_name: string }>();
  ...
  const loadData = async (isRefresh = false) => {
    try {
      ...
      if (searchQuery && tableConfig.searchableColumns.length > 0) {
        ...
        params.append(
          "search_columns",
          JSON.stringify(tableConfig.searchableColumns),
        );
      }
      ...
      const response = await fetch(
        `${process.env.API_URL_SERVER}/api/dashboard/owner/monitoring/data?${params}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
      ...
      Alert.alert(
        "Error",
        error.message || `Gagal memuat data ${tableConfig.displayName}`,
      );
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const handleExport = async () => {
    try {
      ...
      Alert.alert(
        `Export ${tableConfig.displayName} Data`,
        "Export data dalam format CSV atau JSON?",
        [
          { text: "Cancel", style: "cancel" },
          {
            text: "CSV",
            onPress: () => exportData("csv", token),
          },
          {
            text: "JSON",
            onPress: () => exportData("json", token),
          },
        ],
      );
    } catch (error: any) {
  ...
  const exportData = async (format: string, token: string) => {
    try {
      const params = new URLSearchParams({
        table_name: table_name,
        format: format,
      ...
      const response = await fetch(
        `${process.env.API_URL_SERVER}/api/dashboard/owner/export/data?${params}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
    ...
    return (
      <View style={styles.loadingContainer}>
        ...
        <Text style={styles.loadingText}>
          Memuat data {tableConfig.displayName}...
        </Text>
      </View>
    );
  }
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <View style={styles.titleRow}>
            <Ionicons
              name={tableConfig.icon as any}
              size={24}
              color="#2563EB"
            />
            <Text style={styles.title}>{tableConfig.displayName}</Text>
          </View>
      ...
      <View style={styles.searchBar}>
        <View style={styles.searchContainer}>
          ...
          <TextInput
            ...
            placeholder={`Search ${tableConfig.displayName.toLowerCase()}...`}
            value={searchQuery}
            ...
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>
                {searchQuery || Object.keys(selectedFilters).length > 0
                  ...
                  : `No ${tableConfig.displayName.toLowerCase()} data available`}
              </Text>
            </View>
          }
          ListHeaderComponent={
